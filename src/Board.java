public class Board {
    ...
    public Board(){
        arr = new String[15][15]; //Change to Element After, and once changed to type PIECE, every element will be null
    }
    ...
    public void placePiece(int x, int y, String E) {
        arr[x][y] = E;
        placedPiece.x = x;
        placedPiece.y = y;
    }
    ...
    public int getWords() {
        ...
        firstLetter.y--;
        while(arr[firstLetter.x][firstLetter.y] != null) {
            word.y+=arr[firstLetter.x][firstLetter.y];
            firstLetter.y--;
        }
        word.y.toLowerCase();
        ...
        firstLetter.x++;
        while(arr[firstLetter.x][firstLetter.y] != null) {
            word.x +=arr[firstLetter.x][firstLetter.y];
            firstLetter.x++;
        }
        word.x.toLowerCase();
